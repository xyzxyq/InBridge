<div align="center">
  ...
  <p>
    ...
    ·
    <a href="https://mcp.example.com/">官方网站</a>
    ·
  ...
  <p>
    ...
    <img src="https://img.shields.io/badge/version-0.13.0-8b5cf6" alt="版本 0.13.0" />
...
| 端点 | 方法 | 说明 |
| --- | --- | --- |
| `https://mcp.example.com/` | `GET` | 产品官网与本地交互演示 |
...
普通调用无需先枚举模板：常见意图直接使用 `render_interaction_template`；只有确实需要自定义字段组合时才使用 `ask_user_interactively`。旧入口 `render_interaction` 仅作为应用侧兼容别名保留，不再向模型公开推荐。
...
4. 保存后刷新或重新连接应用；
5. 在新对话中启用 InBridge；
6. 建议将下方指令添加到 ChatGPT「个性化」设置的「自定义指令」中，让 ChatGPT 在合适的任务里自行判断并主动调用。
