...
- MCP：`https://<你的域名>/mcp`
- Vercel 项目：你自己账户中的 InBridge 项目

仓库不提供共享生产地址。Vercel 会通过 `VERCEL_PROJECT_PRODUCTION_URL` 自动提供实例域名；使用自定义域名或其他 Node.js 平台时，设置 `INBRIDGE_PUBLIC_URL=https://<你的域名>`。
...
GitHub Actions `Production monitor` 每 6 小时执行一次完整远程烟雾测试，也可以在 Actions 页面手动触发。先在仓库的 Settings → Secrets and variables → Actions 中添加 Secret `INBRIDGE_BASE_URL`，值为你自己的站点根地址。地址只由工作流运行时读取，不应提交到仓库。它真实检查：
...
- MCP initialize
- `tools/list`
- `ask_user_interactively` 和旧入口的仅应用可见兼容元数据
...
```powershell
vercel logs <你的部署地址> --level error --since 1h
```
